...
class CrudViewCommand extends Command
{
    ...
    protected function createField($item)
    {
        ...
        switch ($this->typeLookup[$item['type']]) {
            ...
            case 'yesno':
                return $this->createSelectYesNoField($item);
            case 'select':
            case 'enum':
    ...
    public function createSelectYesNoField($item)
    {
        $start = $this->delimiter[0];
        $end = $this->delimiter[1];

        $required = $item['required'] ? 'required' : '';
        $markup = File::get($this->viewDirectoryPath . 'form-fields/yesno-field.blade.stub');
