...
class ProcessController extends Controller
{
    ...
    {
        /** Code By HAMDAN **/
        $request->request->add(['fields'=>array_merge($request->request->get('fields'),["status"])]);
        $request->request->add(['fields_type'=>array_merge($request->request->get('fields_type'),["tinyint"])]);
        $request->request->add(['fields_required'=>array_merge($request->request->get('fields_required'),["0"])]);
        /** Code By HAMDAN **/
        ...
        return redirect()->back()->with('success', 'Your CRUD has been generated. See on the menu.');
    }
}
